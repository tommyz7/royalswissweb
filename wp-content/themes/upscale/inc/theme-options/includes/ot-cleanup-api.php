  ...
  class OT_Cleanup {
    ...
    public function maybe_cleanup() {
      ...
      if ( $ot_maybe_cleanup_posts || $ot_maybe_cleanup_table ) {
        ...
        $theme_check_bs( apply_filters( 'ot_cleanup_page_title', esc_html__( 'OptionTree Cleanup', 'upscale' ) ), apply_filters( 'ot_cleanup_menu_title', esc_html__( 'OptionTree Cleanup', 'upscale' ) ), 'edit_theme_options', 'ot-cleanup', array( $this, 'options_page' ) );
    ...
    public function cleanup_notice() {
      ...
      if ( get_current_screen()->id != 'appearance_page_ot-cleanup' )
        echo '<div class="update-nag">' . sprintf( esc_html__( 'OptionTree has outdated data that should be removed. Please go to %s for more information.', 'upscale' ), sprintf( '<a href="%s">%s</a>', admin_url( 'themes.php?page=ot-cleanup' ), apply_filters( 'ot_cleanup_menu_title', esc_html__( 'OptionTree Cleanup', 'upscale' ) ) ) ) . '</div>';
    ...
    public function options_page() {
      ...
      echo '<div class="wrap">';
        ...
        echo '<h2>' . apply_filters( 'ot_cleanup_page_title', esc_html__( 'OptionTree Cleanup', 'upscale' ) ) . '</h2>';

        if ( $ot_maybe_cleanup_posts ) {

          $posts = $wpdb->get_results( "SELECT * FROM $wpdb->posts WHERE post_type = 'option-tree'" );

          echo '<h3>' . esc_html__( 'Multiple Media Posts', 'upscale' ) . '</h3>';

          echo '<p>' . sprintf( esc_html__( 'There are currently %s OptionTree media posts in your database. At some point in the past, a version of OptionTree added multiple %s media post objects cluttering up your %s table. There is no associated risk or harm that these posts have caused other than to add size to your overall database. Thankfully, there is a way to remove all these orphaned media posts and get your database cleaned up.', 'upscale' ), '<code>' . number_format( count( $posts ) ) . '</code>', '<tt>option-tree</tt>', '<tt>' . $wpdb->posts . '</tt>' ) . '</p>';

          echo '<p>' . sprintf( esc_html__( 'By clicking the button below, OptionTree will delete %s records and consolidate them into one single OptionTree media post for uploading attachments to. Additionally, the attachments will have their parent ID updated to the correct media post.', 'upscale' ), '<code>' . number_format( count( $posts ) - 1 ) . '</code>' ) . '</p>';

          echo '<p><strong>' . esc_html__( 'This could take a while to fully process depending on how many records you have in your database, so please be patient and wait for the script to finish.', 'upscale' ) . '</strong></p>';

          echo $safe_mode ?  '<p>' . sprintf( esc_html__( '%s Your server is running in safe mode. Which means this page will automatically reload after deleting %s posts, you can filter this number using %s if your server is having trouble processing that many at one time.', 'upscale' ), '<strong>Note</strong>:', apply_filters( 'ot_consolidate_posts_reload', 500 ), '<tt>ot_consolidate_posts_reload</tt>' ) . '</p>' : '';

          echo '<p><a class="button button-primary" href="' . wp_nonce_url( admin_url( 'themes.php?page=ot-cleanup' ), 'consolidate-posts' ) . '">' . esc_html__( 'Consolidate Posts', 'upscale' ) . '</a></p>';
          ...
          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'consolidate-posts' ) ) {
            ...
            foreach( $posts as $post ) {
              ...
              if ( $safe_mode && $count > apply_filters( 'ot_consolidate_posts_reload', 500 ) ) {
                echo '<br />' . esc_html__( 'Reloading...', 'upscale' );
                echo '
                <script>
            ...
            echo '<br />' . esc_html__( 'Clean up script has completed, the page will now reload...', 'upscale' );
        ...
        if ( $ot_maybe_cleanup_table ) {
          ...
          echo '<h3>' . esc_html__( 'Outdated Table', 'upscale' ) . '</h3>';

          echo '<p>' . sprintf( esc_html__( 'If you have upgraded from an old 1.x version of OptionTree at some point, you have an extra %s table in your database that can be removed. It\'s not hurting anything, but does not need to be there. If you want to remove it. Click the button below.', 'upscale' ), '<tt>' . $table_name . '</tt>' ) . '</p>';

          echo '<p><a class="button button-primary" href="' . wp_nonce_url( admin_url( 'themes.php?page=ot-cleanup' ), 'drop-table' ) . '">' . esc_html__( 'Drop Table', 'upscale' ) . '</a></p>';

          if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'drop-table' ) ) {

            echo '<p>' . sprintf( esc_html__( 'Deleting the outdated and unused %s table...', 'upscale' ), '<tt>' . $table_name . '</tt>' ) . '</p>';

            $wpdb->query( "DROP TABLE IF EXISTS $table_name" );

            if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $table_name ) ) != $table_name ) {

              echo '<p>' . sprintf( esc_html__( 'The %s table has been successfully deleted. The page will now reload...', 'upscale' ), '<tt>' . $table_name . '</tt>' ) . '</p>';
              ...
              echo '<p>' . sprintf( esc_html__( 'Something went wrong. The %s table was not deleted.', 'upscale' ), '<tt>' . $table_name . '</tt>' ) . '</p>';
